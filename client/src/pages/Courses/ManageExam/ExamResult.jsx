...
const ExamResult = () => {
  ...
  const fetchResults = async () => {
    try {
      ...
      console.log("Teacher exam results:", data);
      ...
      console.error("Teacher exam results error:", error);
  ...
  const examOptions = useMemo(() => {
    const exams = results
      .map((result) => getExamTitle(result))
      .filter((exam) => exam !== "N/A");
  ...
  const filteredResults = useMemo(() => {
    ...
    return results.filter((result) => {
      ...
      const matchesExam =
        selectedExam === "all" ||
        getExamTitle(result) === selectedExam;
  ...
  const statistics = useMemo(() => {
    ...
    const passedCount = filteredResults.filter(
      (result) => getStatus(result) === "Passed"
    ).length;

    const failedCount = filteredResults.filter(
      (result) => getStatus(result) === "Failed"
    ).length;

    const totalPercentage = filteredResults.reduce(
      (total, result) =>
        total + getPercentage(result),
      0
    );
